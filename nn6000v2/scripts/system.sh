...
install_radar_sender() {
    local pkg="$BASE_PATH/package/radar-raw1-sender"
    local target="$BUILD_DIR/package/radar-raw1-sender"
    [ -d "$pkg" ] || return 0
    rm -rf "$target"
    mkdir -p "$(dirname "$target")"
    cp -a "$pkg" "$target"
    echo "Installed radar-raw1-sender package"
}
change_dnsmasq2full() {
